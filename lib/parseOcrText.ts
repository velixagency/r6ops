export const parseOcrText = (text: string): any => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lowerLine.includes("food") && !lowerLine.includes("food boost") && !lowerLine.includes("food gather")) {
    ...
    } else if (lowerLine.includes("user name") || lowerLine.includes("username")) {
      const match = lines[i].match(/USER\s*NAME:\s*([^\n]+)/i);
      if (match) {
        data.username = match[1].trim() || '';
      }
      ...
      if (match) {
        data.nation = parseInt(match[1]) || 0;
      }
